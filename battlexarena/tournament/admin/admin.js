...
const firebaseConfig = {
  ...
  measurementId: "G-T823ZFL8DL"
        ...
        async function loadDashboardStats() {
            ...
            // Process Users
            if (results[0].status === 'fulfilled') elements.statTotalUsers.textContent = results[0].value.exists() ? Object.keys(results[0].value.val() || {}).length : 0;
            ...
            // Process Games
            if (results[1].status === 'fulfilled') elements.statTotalGames.textContent = results[1].value.exists() ? Object.keys(results[1].value.val() || {}).length : 0;
            else { console.error("Err Games:", results[1].reason); setError(elements.statTotalGames); }

            // Process Promotions
            if (results[2].status === 'fulfilled') elements.statTotalPromotions.textContent = results[2].value.exists() ? Object.keys(results[2].value.val() || {}).length : 0;
            ...
            if (results[4].status === 'fulfilled') {
                const count = results[4].value.exists() ? Object.keys(results[4].value.val() || {}).length : 0;
            ...
            if (results[5].status === 'fulfilled') elements.statCompletedWithdrawals.textContent = results[5].value.exists() ? Object.keys(results[5].value.val() || {}).length : 0;
            ...
            if (results[6].status === 'fulfilled') elements.statRejectedWithdrawals.textContent = results[6].value.exists() ? Object.keys(results[6].value.val() || {}).length : 0;
        ...
        async function loadPromotions() {
             ...
             try {
                 ...
                 let tableHtml = '';
                 if (snapshot.exists()) {
                    promoCount = Object.keys(snapshot.val() || {}).length;
                     snapshot.forEach(childSnapshot => {
        ...
        function setupRealtimeAdminListeners() {
            ...
            dbListeners[countKey] = onValue(pendingQuery, (snapshot) => {
                 const count = snapshot.exists() ? Object.keys(snapshot.val() || {}).length : 0; console.log("Realtime pending withdrawal count:", count);
                 if (elements.pendingWithdrawalCountBadge) { elements.pendingWithdrawalCountBadge.textContent = count; elements.pendingWithdrawalCountBadge.style.display = count > 0 ? 'inline-block' : 'none'; }
                 if (elements.dashboardSection?.classList.contains('active') && elements.statPendingWithdrawals) elements.statPendingWithdrawals.textContent = count;
            }, error => {
